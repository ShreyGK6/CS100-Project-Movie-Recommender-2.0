...
In terms of SOLID principles, the design adheres to these principles for enhanced software design and maintainability. Breaking down the
 SOLID principles:
- Single Responsibility Principle has been applied to the Settings class and seems to handle user preferences and authentication. By separating the concerns of user preferences (genre, actor, director, Age) and authentication (username, password), Settings class adheres to the SRP. The changeprefs() method is responsible for modifying user preferences, keeping the class focused on a single responsibility. This separation allows for better maintenance and future modifications. If the authentication logic or user preferences handling needs to change, it can be done independently without affecting the other.
- Next the Open/Closed Principle has been applied to the start() function where this initiates the sign-in process. You would be able to add more preferences easily, for example by release year. If the sign-in process evolves or new features are added, adhering to the OCP means you can extend the class without modifying existing code. It makes the class more maintainable and extensible. Future changes or additions to the sign-in process won't require modifying existing code. Other areas where the
- Interface Segregation Principle has been applied to the instructions() and display() functions, because this class provides instructions and displays information. If there are multiple implementations for instructions or display in the future, breaking this class into smaller interfaces adheres to ISP.  It makes the class more adaptable to changes. If a developer needs to add or modify instructions or display behavior, they can do so without affecting the entire class. Moreover the Interface Segregation Principle has been applied to the preferences function, setactor() and setdirector(). If the user doesn’t want to input these preferences in, then they won’t get recommendations from these categories. This follows ISP because the set functions also don’t inhibit any other code and doesn’t cause detriment to the program. 
- Lastly, the last SOLID principle that was implemented is the Dependency Inversion Principle to the Database class which uses vectors of Movie objects. By abstracting the storage mechanism behind an interface for example loadFromTSV(), there is an inversion dependency. This allows for easier substitution of different storage implementations without affecting the higher-level Database class. It makes the code more flexible and adaptable to changes in the data storage mechanism. If the developer switches from a TSV file to a database, they can do so without modifying the Database class.
